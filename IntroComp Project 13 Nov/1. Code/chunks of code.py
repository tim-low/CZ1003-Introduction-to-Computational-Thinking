...
def ask(screen):
  pygame.font.init()
  current_string = []
  display_textbox(screen, "Enter an Outlet, Food, or Price Range" + ": " + "".join(current_string))
  while True:
    string = ""
    inkey = get_key()
    ...
    display_map()
    display_textbox(screen, "Enter an Outlet, Food, or Price")
    display_textbox(screen, "Enter an Outlet, Food, or Price" + ": " + "".join(current_string))
    count = 0
...
def display_buttons():
    ...
    screen.fill((200, 200, 200))
    screen.blit(pygame.image.load("NTU map.png"), (0,0))
    pygame.draw.rect(screen, (0,0,0),(x1,y1,width,height), 0)
    pygame.draw.rect(screen, (255,255,255),(x1,y1,width,height), 1)
    pygame.draw.rect(screen, (0,0,0),(x2,y2,width,height), 0)
    pygame.draw.rect(screen, (255,255,255),(x2,y2,width,height), 1)
    pygame.display.flip()
...
while run:
    ...
    for event in events:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if True:
                print(sort_distance(mouse_pos, NTU_Food)[0:6]) #--> Display Text
            
            #If user clicks on the "Sort by Rank" button
            if True:
                print(sort_by_rank(rank)) #--> Display Text
